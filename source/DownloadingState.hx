...
class DownloadingState extends MusicBeatState
{
    ...
    {
        Application.current.window.alert('There was an error in the process of saving the game files. Unfortunately, we are unable to give any error info. So plase send this JSON over to Discussions at Discord.', "Error Saving File");
        ...
        switch (stuffToDownload[currentPos].type) //check if its already downloaded/exists in game files
        {
            case 'charJson': 
                ...
                if (CacheShit.jsons[path] != null)
                    return true;
            case 'charImage': 
                path = "assets/images/custom_chars/" + stuffToDownload[currentPos].name + "/char.png";
                if (CacheShit.images[path] != null)
                    return true;
            case 'charXml': 
                path = "assets/images/custom_chars/" + stuffToDownload[currentPos].name + "/char.xml";
                if (CacheShit.xmls[path] != null)
                    return true;
            case 'icon': 
                path = "assets/images/betterfusion_customize/icons/" + stuffToDownload[currentPos].name + "/icon.png";
                if (CacheShit.images[path] != null)
                    return true;

            case 'stageJson': 
                path = 'assets/images/custom_stages/' + PlayState.SONG.stage + '/stageData.json';
                if (CacheShit.jsons[path] != null)
                    return true;
            case 'pieceJson': 
            ...
            case 'modchart': 
                path = "assets/scripts/" + PlayState.SONG.song.toLowerCase() + "/script.hscript";
