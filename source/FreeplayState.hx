...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var json:FreeplayJson = CoolUtil.parseJson(File.getContent('assets/data/freeplaySongJson.jsonc'));

		trace(json.songs 	+ ' | ' + json.icons + ' | ');

		for (i in 0...json.songs.length)
		{
			songs.push(new SongMetadatas(json.songs[i], 1, json.icons[i])); 
